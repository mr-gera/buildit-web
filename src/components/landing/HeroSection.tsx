...
import { heroFeatures, heroScreens } from '@/data/landing'
      ...
      <Container className="grid grid-cols-1 items-center gap-12 lg:grid-cols-2 lg:gap-10">
        ...
        <div className="flex flex-col gap-6">
          ...
          <h1 className="text-4xl font-extrabold leading-[1.1] tracking-tight text-navy sm:text-5xl lg:text-[3.25rem]">
            Construction progress control from task to{' '}
            <span className="text-orange">photo proof</span>
          </h1>

          <p className="max-w-xl text-lg text-text-muted">
            eBud helps owners, foremen and workers manage renovation and
            construction projects, track statuses, assign tasks, and keep photo
          ...
          <div className="flex max-w-xl items-start gap-4 rounded-2xl bg-blue-soft p-5">
            ...
            <p className="text-sm text-navy/80">
              <span className="font-semibold text-navy">eBud</span> is currently
              in development. We’re building the future of construction
              management.
            </p>
          </div>
        </div>
    ...
    <div className="relative mx-auto w-full max-w-md lg:max-w-none">
      <div className="relative flex items-center justify-center">
        ...
        <div className="absolute left-0 top-8 z-10 w-[38%] -rotate-6 sm:w-[40%]">
          <PhoneMockup src={heroScreens[0].image} alt={heroScreens[0].alt} />
        </div>

        {/* right phone */}
        <div className="absolute right-0 top-8 z-10 w-[38%] rotate-6 sm:w-[40%]">
          <PhoneMockup src={heroScreens[2].image} alt={heroScreens[2].alt} />
        </div>

        {/* center phone (front) */}
        <div className="relative z-20 w-[44%] sm:w-[46%]">
          <PhoneMockup src={heroScreens[1].image} alt={heroScreens[1].alt} />
        </div>
      </div>
    </div>
  )
